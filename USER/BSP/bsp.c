...
#define HSE_VALUE    ((uint32_t)8000000) //定义外部HSE高速时钟
/*
*********************************************************************************************************
*                                            LOCAL DEFINES
*********************************************************************************************************
...
void  BSP_Init (void)
{
	...
	BSP_Tick_Init();         /* Initialize Tick Services.  调用 SysTick                          */
	LED_Init();            //初始化 LED 
	Rheostat_Init();	//初始化ADC  
	EXTI_Key_Config();//KEY1中断模式
	//Key1_GPIO_Config();//KEY1非中断模式
	ttp226init();
	EC20PWFoot_Config();//ec20开关控制
	//EXTI_RI_FOOT_PIN_Config();
	LINKEC20_USART_ALL_Init();
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_CRC, ENABLE); //打开此功能,才能使用STMWIN_GUI
	sFLASH_Init();
	My_RTC_Init();		 		//初始化RTC
	my_LCD_Init();
