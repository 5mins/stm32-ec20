 ...
 * @retvalue   :None
********************************************************************/
void GUIBASE_DrawPoint(u16 x,u16 y,u16 color)
{
...
{  
	...
	{		
		for(pos=0;pos<size;pos++)
		{
		    ...
		    {                 
		        if(temp&0x01){
							Lcd_WriteData_16Bit(fc);	//printf("*"); //debug_mark
							}//如果最高位为 1，输出"*"号,表示笔迹	} 
						else {
						Lcd_WriteData_16Bit(bc);	//printf(" ");	//debug_mark
						} 
				temp>>=1; 
...
void GUI_DrawFont16(u16 x, u16 y, u16 fc, u16 bc, u8 *s,u8 mode)
{
	...
	macGetGBKCode (ucBuffer, s); //取字模数据到ucBuffer；
	...
	for(i=0;i<16*2;i++)
	{
		for(j=0;j<8;j++)
			{	
				if(!mode) //非叠加方式
				{
					if(ucBuffer[i]&(0x80>>j))	{
						Lcd_WriteData_16Bit(fc); //printf("*"); 
					}//0x80==10000000  如果最高位为 1，输出前景色,表示笔迹 //debug_mark
					else {
					Lcd_WriteData_16Bit(bc);		//printf(" ");
					}
				}
				else
				{
	...
	for(i=0;i<16*2;i++)
	{
		for(j=0;j<8;j++)
			{	
				if(!mode) //非叠加方式
				{
					if(ucBuffer[i]&(0x80>>j))	Lcd_WriteData_16Bit(fc);//0x80==10000000  如果最高位为 1，输出前景色,表示笔迹
					else Lcd_WriteData_16Bit(bc);
				}
				else
				{
					POINT_COLOR=fc;
						if(ucBuffer[i]&(0x80>>j))	LCD_DrawPoint(x,y);//画一个点
						x++;
						if((x-x0)==16)
						{
							x=x0;
...
////////////////
void Gui_DrawbmpSign(u16 x,u16 y,const unsigned char *p) //显示15*12 信号值图标
{
  int i; 
	unsigned char picH,picL; 
	LCD_SetWindows(x,y,x+15-1,y+12-1);//窗口设置
    for(i=0;i<15*12;i++)
	{	
	 	picL=*(p+i*2);	//数据低位在前
		picH=*(p+i*2+1);				
...
////////////////
void Gui_DrawbmpWeek(u16 x,u16 y,const unsigned char *p) //显示15*11 星期图标
{
  int i; 
	unsigned char picH,picL; 
	LCD_SetWindows(x,y,x+15-1,y+11-1);//窗口设置
    for(i=0;i<15*13;i++)
	{	
	 	picL=*(p+i*2);	//数据低位在前
		picH=*(p+i*2+1);				
...
//size:字体大小，displaylen：每次显示的字数，mode模式：0覆盖。
void DisplayMovStr(u16 x, u16 y, u16 fc, u16 bc, u8 *str,u8 size,u8 displaylen,u8 mode)
{
	
	u8 strat_xpot=16,i;
	u16 dislen=8;
	u8 strsize = strlen((char*)str);
	
	if(strsize<dislen){dislen=strsize;}
	u8 distimes = (strsize-dislen);
	//printf("strlen:%d\r\n",strsize);//debug_mark
	...
	if(distimes>0)
		{
			...
			while(distimes){
			LCD_Fill(x+strat_xpot,y,x+strat_xpot+(dislen*16) ,y+16,bc); 
			for(i=0;i<strat_xpot;i--){
				Show_Str(x+strat_xpot, y, fc, bc,str,size,mode);
				delay_ms(100);
				}
				//printf("str:%s\r\n",str);//debug_mark
				if(*str!=0)//数据未结束
				{
				if(*str>0x80){str+=2;distimes--;}//中文
				else{str++;}
				}
			distimes--;
			...
			{
				Show_Str(x+6, y, fc, bc,str,size,mode);
				delay_ms(1000);
			}
		LCD_Fill(x,y, lcddev.width-1,y+16,BACK_COLOR); 
}
