...
{ 
     ...
     uint32_t pos;
	  ...
	  High8bit= *c ;//>> 8;     /* 取高8位数据 */
    Low8bit= *(c+1) ;//& 0x00FF;  /* 取低8位数据 */		

//	  	
		/*GB2312 公式*/
    pos = (((High8bit-0xa1)*94)+(Low8bit-0xa1))*macWIDTH_CH_CHAR*macHEIGHT_CH_CHAR/8;
		sFLASH_ReadBuffer(pBuffer,GBKCODE_START_ADDRESS+pos,macWIDTH_CH_CHAR*macHEIGHT_CH_CHAR/8); //读取字库数据
//		Sent_to_DEBUG_with_Block(pBuffer,32);
