...
#include "./bsp/1.44TFT/LCD_drive_hard.h"      /* private modul definitions & config */
...
#ifndef LCD_ON
	#define LCD_ON()  LCD_LED=1;//点亮背光
#endif

#ifndef LCD_OFF
	#define LCD_OFF()  LCD_LED=0;//关闭背光
#endif
...
void LCD_L0_SetPixelIndex(int x, int y, int PixelIndex) {
  ...
  {
   LCD_DrawPoint2(x,y,PixelIndex);//PixelIndex 像素颜色索引
  }
}
...
unsigned int LCD_L0_GetPixelIndex(int x, int y) {
  ...
  {
		   /////////
    //PixelIndex = LCD_ReadPoint(x,y);/* ... */  不需要读点的颜色
  }
  return PixelIndex;
}
