...
void HSE_SetSysClock(uint32_t m, uint32_t n, uint32_t p, uint32_t q);//debug_mark
...
void SystemInit(void)
{
  ...
  SetSysClock();//debug_mark 修改系统时钟实验
	//HSE_SetSysClock(8, 336, 2, 7);//debug_mark 修改系统时钟实验
...
* SYSCLK=PLLCLK=HSE/m*n/p
	HSE_SetSysClock(8, 336, 2, 7);// 系统设定为168MHZ= n/p   OTG FS需要48MHZ,所以q=336/48=7
 * 系统时钟超频到 216M
...
/******************************************************************************/
  ...
  /* Wait till HSE is ready and if Time out is reached exit */// 等待 HSE 启动稳定
  do
  {
